      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl p-5">
              <h3 className="text-sm text-app-green font-semibold mb-3">Items ({order?.items.length})</h3>
              <div className="space-y-3">
                {order?.items.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <img src={item.image} alt={item.name} className="size-10 rounded-lg object-cover" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium scrollbar-track-app-green truncate">{item.name}</p>
              ...
              <div className="mt-4 pt-3 border-t border-app-border space-y-1.5 text-sm">
                <div className="flex justify-between">
                  <span className="text-app-text-light">Subtotal</span><span>{currency}{order?.subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-app-text-light">Delivery</span><span>{order?.deliveryFee === 0 ? "Free" : `${currency}${order?.deliveryFee.toFixed(2)}`}</span>
                </div>
              </div>
            </div>
          </div>
