        ...
        <nav className="bg-white sticky top-0 z-50 border-b border-app-border ">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 gap-4">
                <Link to="/" className="flex items-center gap-2 text-[22px] font-medium shrink-0">
                    <IconLeaf size={24} /> freshcart
                </Link>
                <div className="w-full flex items-center justify-end gap-4 lg:gap-10">
                    <div className="hidden md:flex items-center gap-6 text-sm text-app-text">
                        <Link to="/">Home</Link>
                    ...
                    <form onSubmit={handleSearch} className="hidden sm:flex flex-1 max-w-sm text-xs sm:text-sm">
                        <div className="relative w-full">
                            ...
                            <input
                                ...
                                className="w-full pl-8 p-2 bg-orange-50 rounded-lg ring ring-app-orange/15 focus:ring-app-orange/30"
                            />
                        </div>
                    </form>
                    <div className="flex items-center gap-3">
                        <button className="relative p-2 rounded-xl" onClick={() => setIsCartOpen(true)}>
                            ...
                            {cartCount > 0 && <span className="absolute -top-1 -right-1 text-[10px] size-4 bg-app-orange text-white rounded-full flex-center">{cartCount}</span>}
                        </button>
                        <div className="relative">
                            {user ? (
                                <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center p-2 gap-2">
                                    <div className="size-6 flex-center bg-green-950 rounded-full text-white">
                                ...
                                <div className="flex-center gap-2">
                                    <Link to="/login" className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-950 rounded-full hover:bg-app-green-light transition-colors">
                                        ...
                                        <div onClick={() => setUserMenuOpen(false)}>
                                            ...
                                            {user && (
                                                <div className="border-t border-app-border pt-1">
                                                    <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-2.5 text-sm text-app-error hover:bg-red-50 transition-colors w-full">
