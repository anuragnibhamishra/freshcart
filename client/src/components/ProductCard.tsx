    ...
    return (
        <div className="bg-white rounded-2xl overflow-hidden shadow hover-shadow-md transition-all duration-300 group animate-fade-in cursor-pointer " onClick={() => navigate(`/products/${product.id}`)}>
            <div className="relative aspect-square overflow-hidden">
                <img src={product.image} alt={product.name} loading="lazy" decoding="async" className="w-full h-full object-cover p-4 group-hover:p-2 transistion-all duration-300 " />
                <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                    {product.discount > 0 && <span className=" px-2 py-0.5 text-[10px] font-semibold bg-app-orange text-white rounded-full " >{product.discount}% off</span>}
                </div>
            </div>
            <div className="p-3.5 text-zinc-700">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={(e)=>{e.stopPropagation() ; addToCart(product)}} className="size-7 rounded-full bg-app-orange text-white flex-center shrink-0 hover:bg-app-orange-dark active:scale-95 transition-colors">
